...
export default {
  ...
  methods: {
    ...
    info(id) {
      this.$http({
        url: `discussyiyuantanpaifang/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `discussyiyuantanpaifang/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.discussyiyuantanpaifangCrossAddOrUpdateFlag = false;
						 this.parent.search();
